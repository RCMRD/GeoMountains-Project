...
getwd()
setwd('C:/Users/USER/Desktop/Anita2023/speciesModeling/changesSingleSpecies/geoMountains/workdir_2')
...
#********************************************************************************************************************************************************************
#-----------------------------OBTAIN THE EXISTING BIOPHYSICAL VARIABLES IN THE 3 VARIABLES
#********************************************************************************************************************************************************************
biophys_var_Cur
biophys_var_2.6
biophys_var_8.5

# Only population is common, for the current scenario, the mean and standard deviation of NDVI was computed in ArcMap
#****** Convert TIFF to ASCII format or Directly skip this stage and read in the ASCII files
meanNDVI <- raster("../data/biophysical_var/current/mean_ndvi_ca.tif")
sdNDVI <- raster("../data/biophysical_var/current/sd_ndvi_ca.tif")
#****** Write the clipped raster data to ASC format using the raster package
writeRaster(meanNDVI, filename = "../data/biophysical_var/current/meanNDVI", format = "ascii")
writeRaster(sdNDVI, filename = "../data/biophysical_var/current/sdNDVI", format = "ascii")

#****** List the current & converted ASC files in the directory
biophys_var_Cur <-  list.files("../data/biophysical_var/current", pattern = ".asc", full.names = TRUE)
biophys_var_Cur
...
#********************************************************************************************************************************************************************
#-----------------------------DIMENSION, EXTENT & RESOLUTION CHECKS
#********************************************************************************************************************************************************************
# ******  Perform a full check on the resolution & dimensions (To solve the error of different extent)
# ------ Dimensions check  ------ (The dimensions were similar, thus the lines were commented out)
# dim(raster(biophys_var_Cur[[1]])) 
# dim(raster(biophys_var_Cur[[2]]))
# dim(raster(biophys_var_Cur[[3]]))
# dim(raster(biophys_var_Cur[[4]]))
# dim(raster(biophys_var_Cur[[5]]))
# dim(raster(biophys_var_Cur[[6]]))
# dim(raster(biophys_var_Cur[[7]]))
# ------ Resolution check ------ (The resolutions were similar, thus the lines were commented out)
# res(raster(biophys_var_Cur[[1]]))
# res(raster(biophys_var_Cur[[2]]))
# res(raster(biophys_var_Cur[[3]]))
# res(raster(biophys_var_Cur[[4]]))
# res(raster(biophys_var_Cur[[5]]))
# res(raster(biophys_var_Cur[[6]]))
# res(raster(biophys_var_Cur[[7]]))
# ------ Extent by min & max values ------ 
extent(raster(biophys_var_Cur[[1]]))
extent(raster(biophys_var_Cur[[2]]))
extent(raster(biophys_var_Cur[[3]]))
extent(raster(biophys_var_Cur[[4]]))
extent(raster(biophys_var_Cur[[5]])) #This raster file had a different X estent. All other rasters would have to be modified to fit its extent
extent(raster(biophys_var_Cur[[6]]))
extent(raster(biophys_var_Cur[[7]]))


# ------ Store the rasters as variables ------ 
...
sdNDVI <- raster(biophys_var_Cur[[7]])
...
write.csv(Opuntia_models_scores, file.path(outPath, "csvs/Opuntia_2.0/Opuntia_models_scores.csv"), row.names = FALSE)
...
ggsave(file.path(outPath, "plots/Opuntia_2.0/plotByALGO.png"), plot = plotByALGO$plot, width = 8, height = 6)
# Export the statistics as a csv file (e.g., CSV)
write.csv(plotByALGO$tab, file.path(outPath, "csvs/Opuntia_2.0/plotByALGO.csv"), row.names = FALSE)
...
plotByRUN
# Export the plot as an image file (e.g., PNG)
ggsave("../outputs/plots/Opuntia_2.0/plotByRUN.png", plot = plotByRUN$plot, width = 8, height = 6)
# Export the statistics as a csv file (e.g., CSV)
write.csv(plotByRUN$tab, file.path(outPath, "csvs/Opuntia_2.0/plotByRUN.csv"), row.names = FALSE)
...
plotByPA
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/plotByPA.png"), plot = plotByPA$plot, width = 8, height = 6)
# Export the statistics as a csv file (e.g., CSV)
write.csv(plotByPA$tab, file.path(outPath, "csvs/Opuntia_2.0/plotByPA.csv"), row.names = FALSE)
...
# Export the statistics as a csv file (e.g., CSV)
write.csv(Opuntia_models_var_import, file.path(outPath, "csvs/Opuntia_2.0/Opuntia_models_variable_importance.csv"), row.names = FALSE)
...
# Export the statistics as a csv file (e.g., CSV)
write.csv(mean_VariableImportance, file.path(outPath, "csvs/Opuntia_2.0/Opuntia_models_mean_VariableImportance.csv"), row.names = FALSE)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/Generalized Linear Model.png"), plot = glm_eval_strip$plot, width = 12, height = 8)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/Generalized Boosted Regression Model.png"), plot = gbm_eval_strip$plot, width = 12, height = 8)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/Random Forest Model.png"), plot = rf_eval_strip$plot, width = 12, height = 8)
...
# ---- Write the selected data to a CSV file under the specified directory
write.csv(Opuntia_models_selection_scores, file.path(outPath, "csvs/Opuntia_2.0/Opuntia_models_selection_scores.csv"), row.names = FALSE)
...
plotSelectByALGO                          
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/plotSelectByALGO.png"), plot = plotSelectByALGO$plot, width = 8, height = 6)
# Export the statistics as a csv file (e.g., CSV)
write.csv(plotSelectByALGO$tab, file.path(outPath, "csvs/Opuntia_2.0/plotSelectByALGO.csv"), row.names = FALSE)
...
plotSelectByRUN
# Export the plot as an image file (e.g., PNG)
ggsave("../outputs/plots/Opuntia_2.0/plotSelectByRUN.png", plot = plotSelectByRUN$plot, width = 8, height = 6)
# Export the statistics as a csv file (e.g., CSV)
write.csv(plotSelectByRUN$tab, file.path(outPath, "csvs/Opuntia_2.0/plotSelectByRUN.csv"), row.names = FALSE)
...
plotSelectByPA
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/plotSelectByPA.png"), plot = plotSelectByPA$plot, width = 8, height = 6)
# Export the statistics as a csv file (e.g., CSV)
write.csv(plotSelectByPA$tab, file.path(outPath, "csvs/Opuntia_2.0/plotSelectByPA.csv"), row.names = FALSE)
...
# Export the statistics as a csv file (e.g., CSV)
write.csv(Opuntia_models_selection_var_import, file.path(outPath, "csvs/Opuntia_2.0/Opuntia_models_selection_variable_importance.csv"), row.names = FALSE)
...
# Export the statistics as a csv file (e.g., CSV)
write.csv(mean_VariableImportance_selection, file.path(outPath, "csvs/Opuntia_2.0/Opuntia_models_selection_mean_VariableImportance.csv"), row.names = FALSE)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/Random Forest Model (selection).png"), plot = rf_eval_selection$plot, width = 12, height = 8)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/Generalized Boosted Regression Model (selection).png"), plot = gbm_eval_selection$plot, width = 12, height = 8)
...
bioPhys_KE_2050_BC26 <- stack(
                              c(
                                ndvi_1 = "../data/biophysical_var/2050_26/meanndviextent_conservancies_final.asc",
                                pop_pul = "../data/biophysical_var/2050_26/ken_pop_pul_2012_conservancies_final.asc",                          
                                ndvi_2 = "../data/biophysical_var/2050_26/stdndvi_extent_conservancies_final.asc"
                               )
                             )
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/RCP 2.6 (caMEANwm).png"), plot = emCAmeanWM_2.6, width = 12, height = 8)

#------------------------------------------------------------------------------
## Load 2050 biophysical variables (RCP 8.5)
bioPhys_KE_2050_BC85 <- stack(
                              c(
                                ndvi_1 = "../data/biophysical_var/2050_85/meanndviextent_conservancies_final.asc",
                                pop_pul = "../data/biophysical_var/2050_85/ken_pop_pul_2012_conservancies_final.asc",                          
                                ndvi_2 = "../data/biophysical_var/2050_85/stdndvi_extent_conservancies_final.asc"
                               )
                             )
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/RCP 8.5 (caMEANwm).png"), plot = emCAmeanWM_8.5, width = 12, height = 8)
...
write.csv(SRC_current_2050_BC26$Compt.By.Models, file.path(outPath, "csvs/Opuntia_2.0/SRC_current_2050_BC26-Compt.By.Models.csv"), row.names = FALSE)
...
write.csv(SRC_current_2050_BC85$Compt.By.Models, file.path(outPath, "csvs/Opuntia_2.0/SRC_current_2050_BC85-Compt.By.Models.csv"), row.names = FALSE)
...
ggsave(file.path(outPath, "plots/Opuntia_2.0/RCP 2.6 (DiffByPixel).png"), plot = diffByPixel2.6, width = 8, height = 6)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/RCP 8.5 (DiffByPixel).png"), plot = diffByPixel8.5, width = 8, height = 6)
...
##ggsave(file.path(outPath, "plots/Opuntia_2.0/speciesRangeChangeCountPlot2.6.png"), plot = speciesRangeChangePlots2.6$count, width = 8, height = 6, units = "in")
...
##ggsave(file.path(outPath, "plots/Opuntia_2.0/speciesRangeChangePercentagePlot2.6.png"), plot = speciesRangeChangePlots2.6$percentage, width = 8, height = 6, units = "in")
#-----------SRC models maps representing spatially locations (pixels) lost, stable and gained for each single distribution model
##ggsave(file.path(outPath, "plots/Opuntia_2.0/speciesRangeChangeMapsPlot2.6.png"), plot = speciesRangeChangePlots2.6$maps, width = 8, height = 6, units = "in")
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/meanEvalScores_caliFN.png"), plot = meanEvalScores_caliFN$plot, width = 8, height = 6)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/meanEvalScores_caliAlgo.png"), plot = meanEvalScores_caliAlgo$plot, width = 8, height = 6)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/meanEvalScores_valiFN.png"), plot = meanEvalScores_valiFN$plot, width = 8, height = 6)
...
# Export the plot as an image file (e.g., PNG)
ggsave(file.path(outPath, "plots/Opuntia_2.0/meanEvalScores_valiAlgo.png"), plot = meanEvalScores_valiAlgo$plot, width = 8, height = 6)
